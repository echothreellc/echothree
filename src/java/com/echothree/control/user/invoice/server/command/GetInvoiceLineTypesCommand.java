...
import com.echothree.control.user.invoice.common.form.GetInvoiceLineTypesForm;
...
@Dependent
public class GetInvoiceLineTypesCommand
        extends BasePaginatedMultipleEntitiesCommand<InvoiceLineType, GetInvoiceLineTypesForm> {
    ...
    static {
        COMMAND_SECURITY_DEFINITION = new CommandSecurityDefinition(List.of(
                ...
                new PartyTypeDefinition(PartyTypes.EMPLOYEE.name(), List.of(
                        new SecurityRoleDefinition(SecurityRoleGroups.InvoiceLineType.name(), SecurityRoles.List.name())
                ))
        ));
    ...
    protected Long getTotalEntities() {
        return hasExecutionErrors() ? null : invoiceControl.countInvoiceLineTypesByInvoiceType(invoiceType);
    }
    
    @Override
    protected Collection<InvoiceLineType> getEntities() {
        return hasExecutionErrors() ? null : invoiceControl.getInvoiceLineTypes(invoiceType);
    }
    
    @Override
    protected BaseResult getResult(Collection<InvoiceLineType> entities) {
        ...
        if(entities != null) {
            ...
            if(session.hasLimit(InvoiceLineTypeFactory.class)) {
                result.setInvoiceLineTypeCount(getTotalEntities());
            }

            result.setInvoiceLineTypes(invoiceControl.getInvoiceLineTypeTransfers(userVisit, entities));
        }
