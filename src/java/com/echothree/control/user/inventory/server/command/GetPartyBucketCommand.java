...
import com.echothree.control.user.inventory.common.form.GetPartyBucketForm;
...
@Dependent
public class GetPartyBucketCommand
        extends BaseSingleEntityCommand<PartyBucket, GetPartyBucketForm> {
    ...
    private static final CommandSecurityDefinition COMMAND_SECURITY_DEFINITION = new CommandSecurityDefinition(List.of(
            ...
            new PartyTypeDefinition(PartyTypes.EMPLOYEE.name(), List.of(
                    new SecurityRoleDefinition(SecurityRoleGroups.PartyBucket.name(), SecurityRoles.Review.name())
            ))
    ));
    ...
    public GetPartyBucketCommand() {
        super(COMMAND_SECURITY_DEFINITION, FORM_FIELD_DEFINITIONS, true);
    }

    @Override
    protected PartyBucket getEntity() {
        PartyBucket partyBucket = null;
        ...
        var inventoryBucketType = inventoryBucketTypeLogic.getInventoryBucketTypeByName(this, form.getInventoryBucketTypeName());
        ...
        if(!hasExecutionErrors()) {
            ...
            if(!hasExecutionErrors()) {
                partyBucket = bucketControl.getPartyBucket(party, item, unitOfMeasureType, inventoryCondition, inventoryBucketType);

                if(partyBucket == null) {
                    addExecutionError(ExecutionErrors.UnknownPartyBucket.name(), party.getLastDetail().getPartyName(),
                            form.getItemName(), form.getUnitOfMeasureTypeName(),
                            form.getInventoryConditionName(), form.getInventoryBucketTypeName());
                }
            }
        }
    ...
    @Override
    protected BaseResult getResult(PartyBucket partyBucket) {
        var result = InventoryResultFactory.getGetPartyBucketResult();

        if(!hasExecutionErrors()) {
            result.setPartyBucket(bucketControl.getPartyBucketTransfer(getUserVisit(), partyBucket));
        }
