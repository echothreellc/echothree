...
import com.echothree.control.user.inventory.common.form.GetInventoryDispositionsForm;
...
@Dependent
public class GetInventoryDispositionsCommand
        extends BasePaginatedMultipleEntitiesCommand<InventoryDisposition, GetInventoryDispositionsForm> {
    ...
    static {
        COMMAND_SECURITY_DEFINITION = new CommandSecurityDefinition(List.of(
                ...
                new PartyTypeDefinition(PartyTypes.EMPLOYEE.name(), List.of(
                        new SecurityRoleDefinition(SecurityRoleGroups.InventoryDisposition.name(), SecurityRoles.List.name())
                ))
        ));
    ...
    /** Creates a new instance of GetInventoryDispositionsCommand */
    public GetInventoryDispositionsCommand() {
    ...
    protected Long getTotalEntities() {
        return hasExecutionErrors() ? null
                : inventoryDispositionControl.countInventoryDispositionsByInventoryTransactionType(inventoryTransactionType);
    }

    @Override
    protected Collection<InventoryDisposition> getEntities() {
        return hasExecutionErrors() ? null : inventoryDispositionControl.getInventoryDispositions(inventoryTransactionType);
    }

    @Override
    protected BaseResult getResult(Collection<InventoryDisposition> entities) {
        var result = InventoryResultFactory.getGetInventoryDispositionsResult();
        ...
        if(entities != null) {
            ...
            if(session.hasLimit(InventoryDispositionFactory.class)) {
                result.setInventoryDispositionCount(getTotalEntities());
            }

            result.setInventoryDispositions(inventoryDispositionControl.getInventoryDispositionTransfers(userVisit, entities));
        }
