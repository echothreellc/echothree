...
@Dependent
public class CreateInventoryDispositionCommand
        extends BaseSimpleCommand<CreateInventoryDispositionForm> {
    ...
    static {
        COMMAND_SECURITY_DEFINITION = new CommandSecurityDefinition(List.of(
                ...
                new PartyTypeDefinition(PartyTypes.EMPLOYEE.name(), List.of(
                        new SecurityRoleDefinition(SecurityRoleGroups.InventoryDisposition.name(), SecurityRoles.Create.name())
                ))
        ));
    ...
    /** Creates a new instance of CreateInventoryDispositionCommand */
    public CreateInventoryDispositionCommand() {
        super(COMMAND_SECURITY_DEFINITION, FORM_FIELD_DEFINITIONS, false);
    }
    
    @Override
    protected BaseResult execute() {
        var result = InventoryResultFactory.getCreateInventoryDispositionResult();
        ...
        if(!hasExecutionErrors()) {
            ...
            inventoryDisposition = inventoryDispositionLogic.createInventoryDisposition(this, inventoryTransactionType,
                    inventoryDispositionName, isDefault, sortOrder, getPreferredLanguage(), description, partyPK);
        }

        if(inventoryDisposition != null) {
            result.setEntityRef(inventoryDisposition.getPrimaryKey().getEntityRef());
            result.setInventoryTransactionTypeName(inventoryTransactionType.getLastDetail().getInventoryTransactionTypeName());
            result.setInventoryDispositionName(inventoryDisposition.getLastDetail().getInventoryDispositionName());
        }
