...
public class GetSourcesCommand
        extends BasePaginatedMultipleEntitiesCommand<Source, GetSourcesForm> {
    ...
    static {
        COMMAND_SECURITY_DEFINITION = new CommandSecurityDefinition(List.of(
                ...
                new PartyTypeDefinition(PartyTypes.EMPLOYEE.name(), List.of(
                        new SecurityRoleDefinition(SecurityRoleGroups.Source.name(), SecurityRoles.List.name())
                ))
        ));
    ...
    protected Long getTotalEntities() {
        return sourceControl.countSources();
    }
    ...
    @Override
    protected BaseResult getResult(Collection<Source> entities) {
        var result = OfferResultFactory.getGetSourcesResult();

        if(entities != null) {
            if(session.hasLimit(SourceFactory.class)) {
                result.setSourceCount(getTotalEntities());
            }
