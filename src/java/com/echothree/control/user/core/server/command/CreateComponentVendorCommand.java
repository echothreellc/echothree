...
public class CreateComponentVendorCommand
    ...
    static {
        COMMAND_SECURITY_DEFINITION = new CommandSecurityDefinition(Collections.unmodifiableList(Arrays.asList(
                ...
                new PartyTypeDefinition(PartyTypes.EMPLOYEE.name(), Collections.unmodifiableList(Arrays.asList(
                        new SecurityRoleDefinition(SecurityRoleGroups.ComponentVendor.name(), SecurityRoles.Create.name())
                        )))
                )));
    ...
    protected BaseResult execute() {
        ...
        if(componentVendor != null) {
            var basePK = componentVendor.getPrimaryKey();

            result.setComponentVendorName(componentVendor.getLastDetail().getComponentVendorName());
