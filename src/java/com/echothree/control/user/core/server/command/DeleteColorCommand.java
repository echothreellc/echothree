...
public class DeleteColorCommand
        extends BaseSimpleCommand<DeleteColorForm> {
    ...
    static {
        COMMAND_SECURITY_DEFINITION = new CommandSecurityDefinition(Collections.unmodifiableList(Arrays.asList(
                ...
                new PartyTypeDefinition(PartyTypes.EMPLOYEE.name(), Collections.unmodifiableList(Arrays.asList(
                        new SecurityRoleDefinition(SecurityRoleGroups.Color.name(), SecurityRoles.Delete.name())
                        )))
                )));
    ...
    protected BaseResult execute() {
        ...
        var color = colorControl.getColorByNameForUpdate(colorName);
        
        if(color != null) {
            colorControl.deleteColor(color, getPartyPK());
        } else {
