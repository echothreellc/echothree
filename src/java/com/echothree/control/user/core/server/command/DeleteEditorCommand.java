...
public class DeleteEditorCommand
        extends BaseSimpleCommand<DeleteEditorForm> {
    ...
    static {
        COMMAND_SECURITY_DEFINITION = new CommandSecurityDefinition(Collections.unmodifiableList(Arrays.asList(
                ...
                new PartyTypeDefinition(PartyTypes.EMPLOYEE.name(), Collections.unmodifiableList(Arrays.asList(
                        new SecurityRoleDefinition(SecurityRoleGroups.Editor.name(), SecurityRoles.Delete.name())
                        )))
                )));
    ...
    protected BaseResult execute() {
        ...
        var editor = coreControl.getEditorByNameForUpdate(editorName);
        
        if(editor != null) {
            coreControl.deleteEditor(editor, getPartyPK());
        } else {
