                    ...
                    if(!hasExecutionErrors()) {
                        ...
                        if(EntityAttributeTypes.CLOB.name().equals(entityAttributeTypeName)) {
                                        ...
                                        if(entityClobAttribute == null) {
                                            ...
                                            if(!hasExecutionErrors()) {
                                                if(mimeType.getLastDetail().getEntityAttributeType().getEntityAttributeTypeName().equals(EntityAttributeTypes.CLOB.name())) {
                                                    coreControl.createEntityClobAttribute(entityAttribute, entityInstance, language, form.getClobAttribute(), mimeType, getPartyPK());
                                                } else {
                                                    addExecutionError(ExecutionErrors.InvalidMimeType.name(), mimeType.getLastDetail().getMimeTypeName());
                                                }
                                            }
                                        } else {
                                            addExecutionError(ExecutionErrors.DuplicateEntityClobAttribute.name(),
                            ...
                            addExecutionError(ExecutionErrors.MismatchedEntityAttributeType.name(),
                                    EntityAttributeTypes.CLOB.name(), entityAttributeTypeName);
                        }
                    }
                } else {
