...
import com.echothree.control.user.search.remote.form.SearchSecurityRoleGroupsForm;
import com.echothree.control.user.search.remote.result.SearchResultFactory;
import com.echothree.control.user.search.remote.result.SearchSecurityRoleGroupsResult;
...
public class SearchSecurityRoleGroupsCommand
        extends BaseSearchCommand<SearchSecurityRoleGroupsForm, SearchSecurityRoleGroupsResult> {
    ...
    /** Creates a new instance of SearchSecurityRoleGroupsCommand */
    public SearchSecurityRoleGroupsCommand(UserVisitPK userVisitPK, SearchSecurityRoleGroupsForm form) {
                        ...
                        if(!hasExecutionErrors()) {
                            ...
                            if(!hasExecutionErrors()) {
                                ...
                                if(!hasExecutionErrors()) {
                                    ...
                                    SecurityRoleGroupSearchEvaluator securityRoleGroupSearchEvaluator = new SecurityRoleGroupSearchEvaluator(userVisit,
                                            language, searchType, searchDefaultOperator, searchSortOrder, searchSortDirection, searchUseType);

                                    securityRoleGroupSearchEvaluator.setQ(this, form.getQ());
                                    securityRoleGroupSearchEvaluator.setCreatedSince(createdSince == null ? null : Long.valueOf(createdSince));
                                    securityRoleGroupSearchEvaluator.setModifiedSince(modifiedSince == null ? null : Long.valueOf(modifiedSince));
                                    securityRoleGroupSearchEvaluator.setFields(fields == null ? null : Splitter.on(':').trimResults().omitEmptyStrings().splitToList(fields).toArray(new String[0]));

                                    if(!hasExecutionErrors()) {
                                        result.setCount(securityRoleGroupSearchEvaluator.execute(this));
                                    }
                                }
                            }
                        }
