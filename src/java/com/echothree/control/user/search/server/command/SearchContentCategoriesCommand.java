...
import com.echothree.control.user.search.remote.form.SearchContentCategoriesForm;
import com.echothree.control.user.search.remote.result.SearchContentCategoriesResult;
...
public class SearchContentCategoriesCommand
        extends BaseSearchCommand<SearchContentCategoriesForm, SearchContentCategoriesResult> {
    ...
    /** Creates a new instance of SearchContentCategoriesCommand */
    public SearchContentCategoriesCommand(UserVisitPK userVisitPK, SearchContentCategoriesForm form) {
                        ...
                        if(!hasExecutionErrors()) {
                            ...
                            if(!hasExecutionErrors()) {
                                ...
                                if(!hasExecutionErrors()) {
                                    ...
                                    ContentCategorySearchEvaluator contentCategorySearchEvaluator = new ContentCategorySearchEvaluator(userVisit, language,
                                            searchType, searchDefaultOperator, searchSortOrder, searchSortDirection, searchUseType);

                                    contentCategorySearchEvaluator.setQ(this, form.getQ());
                                    contentCategorySearchEvaluator.setCreatedSince(createdSince == null ? null : Long.valueOf(createdSince));
                                    contentCategorySearchEvaluator.setModifiedSince(modifiedSince == null ? null : Long.valueOf(modifiedSince));
                                    contentCategorySearchEvaluator.setFields(fields == null ? null : Splitter.on(':').trimResults().omitEmptyStrings().splitToList(fields).toArray(new String[0]));

                                    if(!hasExecutionErrors()) {
                                        result.setCount(contentCategorySearchEvaluator.execute(this));
                                    }
                                }
                            }
                        }
