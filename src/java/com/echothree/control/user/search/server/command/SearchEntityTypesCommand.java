...
import com.echothree.control.user.search.remote.form.SearchEntityTypesForm;
import com.echothree.control.user.search.remote.result.SearchEntityTypesResult;
...
public class SearchEntityTypesCommand
    ...
    protected BaseResult execute() {
        SearchEntityTypesResult result = SearchResultFactory.getSearchEntityTypesResult();
        SearchLogic searchLogic = SearchLogic.getInstance();
        SearchKind searchKind = searchLogic.getSearchKindByName(null, SearchConstants.SearchKind_ENTITY_TYPE);
                        ...
                        if(!hasExecutionErrors()) {
                            ...
                            if(!hasExecutionErrors()) {
                                ...
                                if(!hasExecutionErrors()) {
                                    ...
                                    EntityTypeSearchEvaluator entityTypeSearchEvaluator = new EntityTypeSearchEvaluator(userVisit, language, searchType,
                                            searchDefaultOperator, searchSortOrder, searchSortDirection, searchUseType);

                                    entityTypeSearchEvaluator.setQ(this, form.getQ());
                                    entityTypeSearchEvaluator.setCreatedSince(createdSince == null ? null : Long.valueOf(createdSince));
                                    entityTypeSearchEvaluator.setModifiedSince(modifiedSince == null ? null : Long.valueOf(modifiedSince));
                                    entityTypeSearchEvaluator.setFields(fields == null ? null : Splitter.on(':').trimResults().omitEmptyStrings().splitToList(fields).toArray(new String[0]));

                                    if(!hasExecutionErrors()) {
                                        result.setCount(entityTypeSearchEvaluator.execute(this));
                                    }
                                }
                            }
                        }
