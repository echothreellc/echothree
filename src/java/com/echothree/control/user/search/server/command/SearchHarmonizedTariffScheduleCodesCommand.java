...
import com.echothree.model.control.item.server.search.HarmonizedTariffScheduleCodeSearchEvaluator;
...
public class SearchHarmonizedTariffScheduleCodesCommand
        extends BaseSearchCommand<SearchHarmonizedTariffScheduleCodesForm, SearchHarmonizedTariffScheduleCodesResult> {
    ...
    /** Creates a new instance of SearchHarmonizedTariffScheduleCodesCommand */
    public SearchHarmonizedTariffScheduleCodesCommand(UserVisitPK userVisitPK, SearchHarmonizedTariffScheduleCodesForm form) {
        super(userVisitPK, form, null, FORM_FIELD_DEFINITIONS, false);
    }
    
    @Override
    protected BaseResult execute() {
        SearchHarmonizedTariffScheduleCodesResult result = SearchResultFactory.getSearchHarmonizedTariffScheduleCodesResult();
        SearchLogic searchLogic = SearchLogic.getInstance();
        SearchKind searchKind = searchLogic.getSearchKindByName(null, SearchConstants.SearchKind_HARMONIZED_TARIFF_SCHEDULE_CODE);
                        ...
                        if(!hasExecutionErrors()) {
                            ...
                            if(!hasExecutionErrors()) {
                                ...
                                if(!hasExecutionErrors()) {
                                    ...
                                    HarmonizedTariffScheduleCodeSearchEvaluator harmonizedTariffScheduleCodeSearchEvaluator = new HarmonizedTariffScheduleCodeSearchEvaluator(userVisit,
                                            language, searchType, searchDefaultOperator, searchSortOrder, searchSortDirection, searchUseType);

                                    harmonizedTariffScheduleCodeSearchEvaluator.setQ(this, form.getQ());
                                    harmonizedTariffScheduleCodeSearchEvaluator.setCreatedSince(createdSince == null ? null : Long.valueOf(createdSince));
                                    harmonizedTariffScheduleCodeSearchEvaluator.setModifiedSince(modifiedSince == null ? null : Long.valueOf(modifiedSince));
                                    harmonizedTariffScheduleCodeSearchEvaluator.setFields(fields == null ? null : Splitter.on(':').trimResults().omitEmptyStrings().splitToList(fields).toArray(new String[0]));

                                    if(!hasExecutionErrors()) {
                                        result.setCount(harmonizedTariffScheduleCodeSearchEvaluator.execute(this));
                                    }
                                }
                            }
                        }
