...
import com.echothree.control.user.search.common.form.SearchEntityAliasTypesForm;
...
@Dependent
public class SearchEntityAliasTypesCommand
        extends BaseSearchCommand<SearchEntityAliasTypesForm, SearchEntityAliasTypesResult> {
                        ...
                        if(!hasExecutionErrors()) {
                            ...
                            if(!hasExecutionErrors()) {
                                ...
                                if(!hasExecutionErrors()) {
                                    ...
                                    var entityAliasTypeSearchEvaluator = new EntityAliasTypeSearchEvaluator(userVisit, language,
                                            searchType, searchDefaultOperator, searchSortOrder, searchSortDirection, searchUseType);

                                    entityAliasTypeSearchEvaluator.setQ(this, form.getQ());
                                    entityAliasTypeSearchEvaluator.setCreatedSince(createdSince == null ? null : Long.valueOf(createdSince));
                                    entityAliasTypeSearchEvaluator.setModifiedSince(modifiedSince == null ? null : Long.valueOf(modifiedSince));
                                    entityAliasTypeSearchEvaluator.setFields(fields == null ? null : Splitter.on(':').trimResults().omitEmptyStrings().splitToList(fields).toArray(new String[0]));

                                    if(!hasExecutionErrors()) {
                                        result.setCount(entityAliasTypeSearchEvaluator.execute(this));
                                    }
                                }
                            }
                        }
