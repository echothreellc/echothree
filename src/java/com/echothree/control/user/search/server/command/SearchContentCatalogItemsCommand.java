...
import com.echothree.control.user.search.common.form.SearchContentCatalogItemsForm;
import com.echothree.control.user.search.common.result.SearchContentCatalogItemsResult;
import com.echothree.control.user.search.common.result.SearchResultFactory;
import com.echothree.model.control.content.server.search.ContentCatalogItemSearchEvaluator;
...
public class SearchContentCatalogItemsCommand
        extends BaseSearchCommand<SearchContentCatalogItemsForm, SearchContentCatalogItemsResult> {
    ...
    /** Creates a new instance of SearchContentCatalogItemsCommand */
    public SearchContentCatalogItemsCommand(UserVisitPK userVisitPK, SearchContentCatalogItemsForm form) {
        super(userVisitPK, form, null, FORM_FIELD_DEFINITIONS, false);
    }
    
    @Override
    protected BaseResult execute() {
        var result = SearchResultFactory.getSearchContentCatalogItemsResult();
        var searchLogic = SearchLogic.getInstance();
        var searchKind = searchLogic.getSearchKindByName(this, SearchKinds.CONTENT_CATALOG_ITEM.name());
                        ...
                        if(!hasExecutionErrors()) {
                            ...
                            if(!hasExecutionErrors()) {
                                ...
                                if(!hasExecutionErrors()) {
                                    ...
                                    var contentCatalogItemSearchEvaluator = new ContentCatalogItemSearchEvaluator(userVisit, language,
                                            searchType, searchDefaultOperator, searchSortOrder, searchSortDirection, searchUseType);

                                    contentCatalogItemSearchEvaluator.setQ(this, form.getQ());
                                    contentCatalogItemSearchEvaluator.setCreatedSince(createdSince == null ? null : Long.valueOf(createdSince));
                                    contentCatalogItemSearchEvaluator.setModifiedSince(modifiedSince == null ? null : Long.valueOf(modifiedSince));
                                    contentCatalogItemSearchEvaluator.setFields(fields == null ? null : Splitter.on(':').trimResults().omitEmptyStrings().splitToList(fields).toArray(new String[0]));

                                    if(!hasExecutionErrors()) {
                                        result.setCount(contentCatalogItemSearchEvaluator.execute(this));
                                    }
                                }
                            }
                        }
