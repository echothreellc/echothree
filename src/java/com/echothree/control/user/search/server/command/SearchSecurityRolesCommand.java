...
import com.echothree.control.user.search.common.form.SearchSecurityRolesForm;
import com.echothree.control.user.search.common.result.SearchResultFactory;
import com.echothree.control.user.search.common.result.SearchSecurityRolesResult;
import com.echothree.model.control.party.server.logic.LanguageLogic;
import com.echothree.model.control.search.common.SearchConstants;
import com.echothree.model.control.search.server.SearchControl;
import com.echothree.model.control.security.server.search.SecurityRoleSearchEvaluator;
...
public class SearchSecurityRolesCommand
        extends BaseSearchCommand<SearchSecurityRolesForm, SearchSecurityRolesResult> {
    ...
    protected BaseResult execute() {
        SearchSecurityRolesResult result = SearchResultFactory.getSearchSecurityRolesResult();
        SearchLogic searchLogic = SearchLogic.getInstance();
        SearchKind searchKind = searchLogic.getSearchKindByName(null, SearchConstants.SearchKind_SECURITY_ROLE);
                        ...
                        if(!hasExecutionErrors()) {
                            ...
                            if(!hasExecutionErrors()) {
                                ...
                                if(!hasExecutionErrors()) {
                                    ...
                                    SecurityRoleSearchEvaluator securityRoleSearchEvaluator = new SecurityRoleSearchEvaluator(userVisit, language, searchType,
                                            searchDefaultOperator, searchSortOrder, searchSortDirection, searchUseType);

                                    securityRoleSearchEvaluator.setQ(this, form.getQ());
                                    securityRoleSearchEvaluator.setCreatedSince(createdSince == null ? null : Long.valueOf(createdSince));
                                    securityRoleSearchEvaluator.setModifiedSince(modifiedSince == null ? null : Long.valueOf(modifiedSince));
                                    securityRoleSearchEvaluator.setFields(fields == null ? null : Splitter.on(':').trimResults().omitEmptyStrings().splitToList(fields).toArray(new String[0]));

                                    if(!hasExecutionErrors()) {
                                        result.setCount(securityRoleSearchEvaluator.execute(this));
                                    }
                                }
                            }
                        }
