...
import com.echothree.control.user.search.common.form.SearchOffersForm;
...
public class SearchOffersCommand
        extends BaseSearchCommand<SearchOffersForm, SearchOffersResult> {
    ...
    protected BaseResult execute() {
        var result = SearchResultFactory.getSearchOffersResult();
        var searchLogic = SearchLogic.getInstance();
        var searchKind = searchLogic.getSearchKindByName(this, SearchKinds.OFFER.name());
                        ...
                        if(!hasExecutionErrors()) {
                            ...
                            if(!hasExecutionErrors()) {
                                ...
                                if(!hasExecutionErrors()) {
                                    ...
                                    var offerSearchEvaluator = new OfferSearchEvaluator(userVisit,
                                            language, searchType, searchDefaultOperator, searchSortOrder, searchSortDirection, searchUseType);

                                    offerSearchEvaluator.setQ(this, form.getQ());
                                    offerSearchEvaluator.setCreatedSince(createdSince == null ? null : Long.valueOf(createdSince));
                                    offerSearchEvaluator.setModifiedSince(modifiedSince == null ? null : Long.valueOf(modifiedSince));
                                    offerSearchEvaluator.setFields(fields == null ? null : Splitter.on(':').trimResults().omitEmptyStrings().splitToList(fields).toArray(new String[0]));

                                    if(!hasExecutionErrors()) {
                                        result.setCount(offerSearchEvaluator.execute(this));
                                    }
                                }
                            }
                        }
