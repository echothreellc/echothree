...
import com.echothree.control.user.search.common.form.SearchContentCatalogsForm;
import com.echothree.control.user.search.common.result.SearchContentCatalogsResult;
import com.echothree.control.user.search.common.result.SearchResultFactory;
import com.echothree.model.control.content.server.search.ContentCatalogSearchEvaluator;
...
@RequestScoped
public class SearchContentCatalogsCommand
        extends BaseSearchCommand<SearchContentCatalogsForm, SearchContentCatalogsResult> {
                        ...
                        if(!hasExecutionErrors()) {
                            ...
                            if(!hasExecutionErrors()) {
                                ...
                                if(!hasExecutionErrors()) {
                                    ...
                                    var contentCatalogSearchEvaluator = new ContentCatalogSearchEvaluator(userVisit, language,
                                            searchType, searchDefaultOperator, searchSortOrder, searchSortDirection, searchUseType);

                                    contentCatalogSearchEvaluator.setQ(this, form.getQ());
                                    contentCatalogSearchEvaluator.setCreatedSince(createdSince == null ? null : Long.valueOf(createdSince));
                                    contentCatalogSearchEvaluator.setModifiedSince(modifiedSince == null ? null : Long.valueOf(modifiedSince));
                                    contentCatalogSearchEvaluator.setFields(fields == null ? null : Splitter.on(':').trimResults().omitEmptyStrings().splitToList(fields).toArray(new String[0]));

                                    if(!hasExecutionErrors()) {
                                        result.setCount(contentCatalogSearchEvaluator.execute(this));
                                    }
                                }
                            }
                        }
