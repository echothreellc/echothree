...
import com.echothree.model.control.offer.server.search.UseTypeSearchEvaluator;
...
public class SearchUseTypesCommand
        extends BaseSearchCommand<SearchUseTypesForm, SearchUseTypesResult> {
    ...
    /** Creates a new instance of SearchUseTypesCommand */
    public SearchUseTypesCommand(UserVisitPK userVisitPK, SearchUseTypesForm form) {
        super(userVisitPK, form, null, FORM_FIELD_DEFINITIONS, false);
    }
    
    @Override
    protected BaseResult execute() {
        SearchUseTypesResult result = SearchResultFactory.getSearchUseTypesResult();
        SearchLogic searchLogic = SearchLogic.getInstance();
        SearchKind searchKind = searchLogic.getSearchKindByName(null, SearchConstants.SearchKind_USE_TYPE);
                        ...
                        if(!hasExecutionErrors()) {
                            ...
                            if(!hasExecutionErrors()) {
                                ...
                                if(!hasExecutionErrors()) {
                                    ...
                                    UseTypeSearchEvaluator useTypeSearchEvaluator = new UseTypeSearchEvaluator(userVisit,
                                            language, searchType, searchDefaultOperator, searchSortOrder, searchSortDirection, searchUseType);

                                    useTypeSearchEvaluator.setQ(this, form.getQ());
                                    useTypeSearchEvaluator.setCreatedSince(createdSince == null ? null : Long.valueOf(createdSince));
                                    useTypeSearchEvaluator.setModifiedSince(modifiedSince == null ? null : Long.valueOf(modifiedSince));
                                    useTypeSearchEvaluator.setFields(fields == null ? null : Splitter.on(':').trimResults().omitEmptyStrings().splitToList(fields).toArray(new String[0]));

                                    if(!hasExecutionErrors()) {
                                        result.setCount(useTypeSearchEvaluator.execute(this));
                                    }
                                }
                            }
                        }
