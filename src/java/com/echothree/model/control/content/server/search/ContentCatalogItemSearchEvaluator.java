...
import com.echothree.model.control.content.server.analyzer.ContentCatalogItemAnalyzer;
...
public class ContentCatalogItemSearchEvaluator
    ...
    /** Creates a new instance of ContentCatalogItemSearchEvaluator */
    public ContentCatalogItemSearchEvaluator(UserVisit userVisit, Language language, SearchType searchType, SearchDefaultOperator searchDefaultOperator,
        ...
        super(userVisit, searchDefaultOperator, searchType, searchSortOrder, searchSortDirection, searchUseType, ComponentVendors.ECHO_THREE.name(),
                EntityTypes.ContentCatalogItem.name(), IndexTypes.CONTENT_CATALOG_ITEM.name(), language, null);
    ...
    public SortField[] getSortFields(String searchSortOrderName) {
        ...
        if(searchSortOrderName.equals(SearchSortOrders.SCORE.name())) {
            ...
            sortFields = new SortField[]{
                new SortField(IndexFields.itemName.name() + IndexConstants.INDEX_FIELD_VARIATION_SEPARATOR + IndexFieldVariations.sortable.name(), SortField.Type.STRING, reverse)
            };
