...
public class EntityAliasTypeSearchEvaluator
        extends BaseSearchEvaluator {
    
    /** Creates a new instance of EntityAliasTypeSearchEvaluator */
    public EntityAliasTypeSearchEvaluator(UserVisit userVisit, Language language, SearchType searchType, SearchDefaultOperator searchDefaultOperator,
        ...
        super(userVisit, searchDefaultOperator, searchType, searchSortOrder, searchSortDirection, searchUseType, ComponentVendors.ECHO_THREE.name(),
                EntityTypes.EntityAliasType.name(), IndexTypes.ENTITY_ALIAS_TYPE.name(), language, null);
    ...
    public SortField[] getSortFields(String searchSortOrderName) {
        ...
        } else if(searchSortOrderName.equals(SearchSortOrders.ENTITY_ALIAS_TYPE_NAME.name())) {
            sortFields = new SortField[]{
                new SortField(IndexFields.componentVendorName.name() + IndexConstants.INDEX_FIELD_VARIATION_SEPARATOR + IndexFieldVariations.sortable.name(), SortField.Type.STRING, reverse),
                new SortField(IndexFields.entityTypeName.name() + IndexConstants.INDEX_FIELD_VARIATION_SEPARATOR + IndexFieldVariations.sortable.name(), SortField.Type.STRING, reverse),
                new SortField(IndexFields.entityAliasTypeName.name() + IndexConstants.INDEX_FIELD_VARIATION_SEPARATOR + IndexFieldVariations.sortable.name(), SortField.Type.STRING, reverse)
            };
    ...
    public BasicAnalyzer getAnalyzer(final ExecutionErrorAccumulator eea, final Language language) {
        return new EntityAliasTypeAnalyzer(eea, language, entityType);
    }
