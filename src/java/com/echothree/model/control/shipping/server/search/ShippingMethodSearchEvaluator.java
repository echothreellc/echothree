...
package com.echothree.model.control.shipping.server.search;
...
import com.echothree.model.control.index.server.analysis.ShippingMethodAnalyzer;
...
public class ShippingMethodSearchEvaluator
        extends BaseSearchEvaluator {

    /** Creates a new instance of ShippingMethodSearchEvaluator */
    public ShippingMethodSearchEvaluator(UserVisit userVisit, Language language, SearchType searchType, SearchDefaultOperator searchDefaultOperator,
        ...
        super(userVisit, searchDefaultOperator, searchType, searchSortOrder, searchSortDirection, searchUseType, ComponentVendors.ECHO_THREE.name(),
                EntityTypes.ShippingMethod.name(), IndexTypes.SHIPPING_METHOD.name(), language, null);
    ...
    public SortField[] getSortFields(String searchSortOrderName) {
        ...
        } else if(searchSortOrderName.equals(SearchSortOrders.SHIPPING_METHOD_NAME.name())) {
            sortFields = new SortField[]{
                new SortField(IndexFields.shippingMethodName.name() + IndexConstants.INDEX_FIELD_VARIATION_SEPARATOR + IndexFieldVariations.sortable.name(), SortField.Type.STRING, reverse)
            };
    ...
    public Analyzer getAnalyzer(final ExecutionErrorAccumulator eea, final Language language) {
        return new ShippingMethodAnalyzer(eea, language, entityType);
    }
