...
@GraphQLDescription("unit of measure kind use type time object")
@GraphQLName("UnitOfMeasureKindUseTypeTime")
public class UnitOfMeasureKindUseTypeTimeObject
        implements BaseGraphQl {

    private final long time; // Always Present

    public UnitOfMeasureKindUseTypeTimeObject(final long time) {
        this.time = time;
    }
    ...
    public String getTime(final DataFetchingEnvironment env) {
        ...
        return UnitOfMeasureUtils.getInstance().formatUnitOfMeasure(BaseGraphQl.getUserVisit(env), timeUnitOfMeasureKind, time);
    }
