...
@SproutAction(
    path = "/v1/GetEntityListItemResults",
    mappingClass = SecureActionMapping.class,
    properties = {
        @SproutProperty(property = "secure", value = "any")
    }
)
public class GetEntityListItemResultsAction
        extends CmsBaseJsonAction {

    /** Creates a new instance of GetEntityListItemResultsAction */
    public GetEntityListItemResultsAction() {
        super(true, false);
    }
    ...
    protected CommandResult getCommandResult(HttpServletRequest request)
        ...
        limits.put(SearchResultConstants.ENTITY_TYPE_NAME,
                new Limit(request.getParameter(ParameterConstants.ENTITY_LIST_ITEM_RESULT_COUNT), request.getParameter(ParameterConstants.ENTITY_LIST_ITEM_RESULT_OFFSET)));
        commandForm.setLimits(limits);

        return SearchUtil.getHome().getEntityListItemResults(getUserVisitPK(request), commandForm);
    }
