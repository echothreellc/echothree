    ...
    <body>
        <div id="Header">
            <h2>
                ...
                <c:url var="filterStepDestinationsUrl" value="/action/Filter/FilterStepDestination/Main">
                    <c:param name="FilterKindName" value="${filterKindName}" />
                    <c:param name="FilterTypeName" value="${filterTypeName}" />
                    <c:param name="FilterName" value="${filterName}" />
                    <c:param name="FilterStepName" value="${filterStepName}" />
                </c:url>
                <a href="${filterStepDestinationsUrl}">Destinations</a> &gt;&gt;
                Add
            </h2>
        </div>
        <div id="Content">
            ...
            <html:form action="/Filter/FilterStepDestination/Add" method="POST">
                <table>
                    <tr>
                        <td align=right><fmt:message key="label.filterStep" />:</td>
                        <td>
                            <html:select property="toFilterStepChoice">
                                <html:optionsCollection property="toFilterStepChoices" />
                            </html:select> (*)
                            <et:validationErrors id="errorMessage" property="ToFilterStepName">
