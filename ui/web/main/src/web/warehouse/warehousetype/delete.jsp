    ...
    <body>
        <div id="Header">
            <h2>
                ...
                Delete (<c:out value="${warehouseType.warehouseTypeName}" />)
            </h2>
        </div>
        <div id="Content">
            <c:choose>
                ...
                <c:otherwise>
                    <p>You are about to delete the <c:out value="${fn:toLowerCase(partyEntityType.entityType.description)}" />
                        <c:choose>
                            <c:when test="${warehouseType.description != null}">&quot;<c:out value="${warehouseType.description}" />.&quot</c:when>
                            <c:otherwise>&quot;<c:out value="${warehouseType.warehouseTypeName}" />.&quot</c:otherwise>
                        </c:choose>
                    </p>
                    <html:form action="/Warehouse/WarehouseType/Delete" method="POST">
                        <table>
                            <tr>
                        ...
                        <html:hidden property="warehouseTypeName" />
                    </html:form>
                </c:otherwise>
            </c:choose>
        </div>
