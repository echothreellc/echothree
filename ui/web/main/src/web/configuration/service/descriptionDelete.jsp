...
<html:html xhtml="true">
    <head>
        <title><fmt:message key="pageTitle.serviceDescriptions" /></title>
        <html:base/>
        ...
        <div id="Header">
            <h2>
                ...
                <c:url var="descriptionsUrl" value="/action/Configuration/Service/Description">
                    <c:param name="ServiceName" value="${serviceDescription.service.serviceName}" />
                </c:url>
                <a href="${descriptionsUrl}"><fmt:message key="navigation.serviceDescriptions" /></a> &gt;&gt;
                Delete
            </h2>
        </div>
        <div id="Content">
            <p>You are about to delete the <c:out value="${serviceDescription.language.description}" />
            <c:out value="${fn:toLowerCase(partyEntityType.entityType.description)}" /> for &quot;<c:out value="${serviceDescription.service.description}" />&quot;
            (<c:out value="${serviceDescription.service.serviceName}" />).</p>
            <et:executionErrors id="errorMessage">
                <p class="executionErrors"><c:out value="${errorMessage}" /></p><br />
            </et:executionErrors>
            <html:form action="/Configuration/Service/DescriptionDelete" method="POST">
                <table>
                    <tr>
