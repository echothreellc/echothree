...
<html:html xhtml="true">
    <head>
        <title><fmt:message key="pageTitle.serverDescriptions" /></title>
        <html:base/>
        ...
        <div id="Header">
            <h2>
                ...
                <c:url var="descriptionsUrl" value="/action/Configuration/Server/Description">
                    <c:param name="ServerName" value="${serverDescription.server.serverName}" />
                </c:url>
                <a href="${descriptionsUrl}"><fmt:message key="navigation.serverDescriptions" /></a> &gt;&gt;
                Delete
            </h2>
        </div>
        <div id="Content">
            <p>You are about to delete the <c:out value="${serverDescription.language.description}" />
            <c:out value="${fn:toLowerCase(partyEntityType.entityType.description)}" /> for &quot;<c:out value="${serverDescription.server.description}" />&quot;
            (<c:out value="${serverDescription.server.serverName}" />).</p>
            <et:executionErrors id="errorMessage">
                <p class="executionErrors"><c:out value="${errorMessage}" /></p><br />
            </et:executionErrors>
            <html:form action="/Configuration/Server/DescriptionDelete" method="POST">
                <table>
                    <tr>
