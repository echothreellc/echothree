...
<html:html xhtml="true">
    <head>
        <title><fmt:message key="pageTitle.contentCollectionDescriptions" /></title>
        <html:base/>
    ...
    <body>
        <div id="Header">
            <h2>
                ...
                <c:url var="descriptionsUrl" value="/action/Content/ContentCollection/Description">
                    <c:param name="ContentCollectionName" value="${contentCollectionDescription.contentCollection.contentCollectionName}" />
                </c:url>
                <a href="${descriptionsUrl}"><fmt:message key="navigation.contentCollectionDescriptions" /></a> &gt;&gt;
                Delete
            </h2>
        </div>
        <div id="Content">
            <p>You are about to delete the <c:out value="${contentCollectionDescription.language.description}" />
            <c:out value="${fn:toLowerCase(partyEntityType.entityType.description)}" /> for &quot;<c:out value="${contentCollectionDescription.contentCollection.description}" />&quot;
            (<c:out value="${contentCollectionDescription.contentCollection.contentCollectionName}" />).</p>
            <et:executionErrors id="errorMessage">
                <p class="executionErrors"><c:out value="${errorMessage}" /></p><br />
            </et:executionErrors>
            <html:form action="/Content/ContentCollection/DescriptionDelete" method="POST">
                <table>
                    <tr>
