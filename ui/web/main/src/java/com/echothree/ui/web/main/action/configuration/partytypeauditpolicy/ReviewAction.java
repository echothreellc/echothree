...
@SproutAction(
    path = "/Configuration/PartyTypeAuditPolicy/Review",
    mappingClass = SecureActionMapping.class,
    properties = {
        @SproutProperty(property = "secure", value = "true")
    },
    forwards = {
        @SproutForward(name = "Display", path = "/configuration/partytypeauditpolicy/review.jsp")
    }
)
public class ReviewAction
    ...
    public ActionForward executeAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
        ...
        PartyTypeAuditPolicyTransfer partyTypeAuditPolicy = partyType == null? null: partyType.getPartyTypeAuditPolicy();
        
        if(partyTypeAuditPolicy == null) {
            forwardKey = ForwardConstants.ERROR_404;
        } else {
            request.setAttribute(AttributeConstants.PARTY_TYPE_AUDIT_POLICY, partyTypeAuditPolicy);
