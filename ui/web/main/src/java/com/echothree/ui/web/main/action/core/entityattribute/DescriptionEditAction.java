...
import com.echothree.control.user.core.remote.edit.EntityAttributeDescriptionEdit;
import com.echothree.control.user.core.remote.form.EditEntityAttributeDescriptionForm;
import com.echothree.control.user.core.remote.result.EditEntityAttributeDescriptionResult;
import com.echothree.control.user.core.remote.spec.EntityAttributeDescriptionSpec;
...
@SproutAction(
    path = "/Core/EntityAttribute/DescriptionEdit",
    mappingClass = SecureActionMapping.class,
    name = "EntityAttributeDescriptionEdit",
    properties = {
        @SproutProperty(property = "secure", value = "true")
    },
    forwards = {
        @SproutForward(name = "Display", path = "/action/Core/EntityAttribute/Description", redirect = true),
        @SproutForward(name = "Form", path = "/core/entityattribute/descriptionEdit.jsp")
    }
)
    ...
    throws Exception {
        ...
        try {
            if(forwardKey == null) {
                ...
                EditEntityAttributeDescriptionForm commandForm = CoreUtil.getHome().getEditEntityAttributeDescriptionForm();
                EntityAttributeDescriptionSpec spec = CoreUtil.getHome().getEntityAttributeDescriptionSpec();
                ...
                if(wasPost(request)) {
                    EntityAttributeDescriptionEdit edit = CoreUtil.getHome().getEntityAttributeDescriptionEdit();
                    
                    commandForm.setEditMode(EditMode.UPDATE);
                    commandForm.setEdit(edit);
                    edit.setDescription(actionForm.getDescription());
                    
                    CommandResult commandResult = CoreUtil.getHome().editEntityAttributeDescription(getUserVisitPK(request), commandForm);
                    
                    if(commandResult.hasErrors()) {
                        ExecutionResult executionResult = commandResult.getExecutionResult();
                        
                        if(executionResult != null) {
                            EditEntityAttributeDescriptionResult result = (EditEntityAttributeDescriptionResult)executionResult.getResult();
                    ...
                    if(result != null) {
                        EntityAttributeDescriptionEdit edit = result.getEdit();
