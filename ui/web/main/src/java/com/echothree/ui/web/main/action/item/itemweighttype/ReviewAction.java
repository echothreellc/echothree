...
@SproutAction(
    path = "/Item/ItemWeightType/Review",
    mappingClass = SecureActionMapping.class,
    properties = {
        @SproutProperty(property = "secure", value = "true")
    },
    forwards = {
        @SproutForward(name = "Display", path = "/item/itemweighttype/review.jsp")
    }
)
public class ReviewAction
    ...
    public ActionForward executeAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
        ...
        commandForm.setItemWeightTypeName(request.getParameter(ParameterConstants.ITEM_WEIGHT_TYPE_NAME));

        var commandResult = ItemUtil.getHome().getItemWeightType(getUserVisitPK(request), commandForm);
        ItemWeightTypeTransfer itemWeightType = null;
        
        if(!commandResult.hasErrors()) {
            var executionResult = commandResult.getExecutionResult();
            var result = (GetItemWeightTypeResult)executionResult.getResult();
            
            itemWeightType = result.getItemWeightType();
        }
        
        if(itemWeightType == null) {
            forwardKey = ForwardConstants.ERROR_404;
        } else {
            request.setAttribute(AttributeConstants.ITEM_WEIGHT_TYPE, itemWeightType);
