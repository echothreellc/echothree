...
@SproutAction(
    path = "/Item/ItemVolumeType/SetDefault",
...
public class SetDefaultAction
    ...
    public ActionForward executeAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        var commandForm = ItemUtil.getHome().getSetDefaultItemVolumeTypeForm();

        commandForm.setItemVolumeTypeName(request.getParameter(ParameterConstants.ITEM_VOLUME_TYPE_NAME));

        ItemUtil.getHome().setDefaultItemVolumeType(getUserVisitPK(request), commandForm);
