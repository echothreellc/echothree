...
import com.echothree.control.user.returnpolicy.remote.form.CreateReturnReasonTypeForm;
...
@SproutAction(
    path = "/ReturnPolicy/ReturnReasonType/Add",
    mappingClass = SecureActionMapping.class,
    name = "ReturnReasonTypeAdd",
    properties = {
        @SproutProperty(property = "secure", value = "true")
    },
    forwards = {
        @SproutForward(name = "Display", path = "/action/ReturnPolicy/ReturnReasonType/Main", redirect = true),
        @SproutForward(name = "Form", path = "/returnpolicy/returnreasontype/add.jsp")
    }
)
    ...
    public ActionForward executeAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if(forwardKey == null) {
                ...
                if(wasPost(request)) {
                    CreateReturnReasonTypeForm commandForm = ReturnPolicyUtil.getHome().getCreateReturnReasonTypeForm();
                    
                    commandForm.setReturnKindName(returnKindName);
                    commandForm.setReturnReasonName(returnReasonName);
                    commandForm.setReturnTypeName(actionForm.getReturnTypeChoice());
                    commandForm.setIsDefault(actionForm.getIsDefault().toString());
                    commandForm.setSortOrder(actionForm.getSortOrder());
                    
                    CommandResult commandResult = ReturnPolicyUtil.getHome().createReturnReasonType(getUserVisitPK(request), commandForm);
