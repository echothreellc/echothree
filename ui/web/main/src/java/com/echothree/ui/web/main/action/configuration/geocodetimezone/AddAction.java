...
@SproutAction(
    path = "/Configuration/GeoCodeTimeZone/Add",
    mappingClass = SecureActionMapping.class,
    name = "GeoCodeTimeZoneAdd",
    properties = {
        @SproutProperty(property = "secure", value = "true")
    },
    forwards = {
        @SproutForward(name = "Display", path = "/action/Configuration/GeoCodeTimeZone/Main", redirect = true),
        @SproutForward(name = "Form", path = "/configuration/geocodetimezone/add.jsp")
    }
)
    ...
    public ActionForward executeAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if(forwardKey == null) {
                ...
                if(wasPost(request)) {
                    CreateGeoCodeTimeZoneForm commandForm = GeoUtil.getHome().getCreateGeoCodeTimeZoneForm();
                    
                    if(geoCodeName == null)
                        geoCodeName = actionForm.getGeoCodeName();
                    
                    commandForm.setGeoCodeName(geoCodeName);
                    commandForm.setJavaTimeZoneName(actionForm.getTimeZoneChoice());
                    commandForm.setIsDefault(actionForm.getIsDefault().toString());
                    commandForm.setSortOrder(actionForm.getSortOrder());
                    
                    CommandResult commandResult = GeoUtil.getHome().createGeoCodeTimeZone(getUserVisitPK(request), commandForm);
