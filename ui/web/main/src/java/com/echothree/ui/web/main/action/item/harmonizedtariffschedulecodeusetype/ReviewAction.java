...
@SproutAction(
    path = "/Item/HarmonizedTariffScheduleCodeUseType/Review",
    mappingClass = SecureActionMapping.class,
    properties = {
        @SproutProperty(property = "secure", value = "true")
    },
    forwards = {
        @SproutForward(name = "Display", path = "/item/harmonizedtariffschedulecodeusetype/review.jsp")
    }
)
public class ReviewAction
    ...
    public ActionForward executeAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
        ...
        HarmonizedTariffScheduleCodeUseTypeTransfer harmonizedTariffScheduleCodeUseType = null;
        
        if(!commandResult.hasErrors()) {
            var executionResult = commandResult.getExecutionResult();
            var result = (GetHarmonizedTariffScheduleCodeUseTypeResult)executionResult.getResult();
            
            harmonizedTariffScheduleCodeUseType = result.getHarmonizedTariffScheduleCodeUseType();
        }
        
        if(harmonizedTariffScheduleCodeUseType == null) {
            forwardKey = ForwardConstants.ERROR_404;
        } else {
            request.setAttribute(AttributeConstants.HARMONIZED_TARIFF_SCHEDULE_CODE_USE_TYPE, harmonizedTariffScheduleCodeUseType);
