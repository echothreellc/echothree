...
@SproutAction(
    path = "/ReturnPolicy/ReturnReason/Add",
    mappingClass = SecureActionMapping.class,
    name = "ReturnReasonAdd",
    properties = {
        @SproutProperty(property = "secure", value = "true")
    },
    forwards = {
        @SproutForward(name = "Display", path = "/action/ReturnPolicy/ReturnReason/Main", redirect = true),
        @SproutForward(name = "Form", path = "/returnpolicy/returnreason/add.jsp")
    }
)
    ...
    throws Exception {
        ...
        try {
            if(forwardKey == null) {
                ...
                if(wasPost(request)) {
                    CreateReturnReasonForm commandForm = ReturnPolicyUtil.getHome().getCreateReturnReasonForm();
                    
                    if(returnKindName == null)
                        returnKindName = actionForm.getReturnKindName();
                    
                    commandForm.setReturnKindName(returnKindName);
                    commandForm.setReturnReasonName(actionForm.getReturnReasonName());
                    commandForm.setIsDefault(actionForm.getIsDefault().toString());
                    commandForm.setSortOrder(actionForm.getSortOrder());
                    commandForm.setDescription(actionForm.getDescription());
                    
                    CommandResult commandResult = ReturnPolicyUtil.getHome().createReturnReason(getUserVisitPK(request), commandForm);
