...
import com.echothree.control.user.item.remote.form.CreateItemWeightForm;
...
@SproutAction(
    ...
    forwards = {
        ...
        @SproutForward(name = "Form", path = "/item/itemweight/add.jsp")
    }
)
public class AddAction
    ...
    public ActionForward executeAction(ActionMapping mapping, AddActionForm form, HttpServletRequest request, HttpServletResponse response)
        ...
        if(wasPost(request)) {
            CreateItemWeightForm commandForm = ItemUtil.getHome().getCreateItemWeightForm();
            ...
            CommandResult commandResult = ItemUtil.getHome().createItemWeight(getUserVisitPK(request), commandForm);
