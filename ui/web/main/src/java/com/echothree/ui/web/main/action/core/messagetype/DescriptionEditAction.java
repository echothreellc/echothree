...
@SproutAction(
    path = "/Core/MessageType/DescriptionEdit",
    mappingClass = SecureActionMapping.class,
    name = "MessageTypeDescriptionEdit",
    properties = {
        @SproutProperty(property = "secure", value = "true")
    },
    forwards = {
        @SproutForward(name = "Display", path = "/action/Core/MessageType/Description", redirect = true),
        @SproutForward(name = "Form", path = "/core/messagetype/descriptionEdit.jsp")
    }
)
    ...
    throws Exception {
        ...
        try {
            if(forwardKey == null) {
                ...
                EditMessageTypeDescriptionForm commandForm = MessageUtil.getHome().getEditMessageTypeDescriptionForm();
                MessageTypeDescriptionSpec spec = MessageUtil.getHome().getMessageTypeDescriptionSpec();
                ...
                if(messageTypeName == null)
                    messageTypeName = actionForm.getMessageTypeName();
                ...
                spec.setMessageTypeName(messageTypeName);
                spec.setLanguageIsoName(languageIsoName);
                
                if(wasPost(request)) {
                    MessageTypeDescriptionEdit edit = MessageUtil.getHome().getMessageTypeDescriptionEdit();
                    
                    commandForm.setEditMode(EditMode.UPDATE);
                    commandForm.setEdit(edit);
                    edit.setDescription(actionForm.getDescription());
                    
                    CommandResult commandResult = MessageUtil.getHome().editMessageTypeDescription(getUserVisitPK(request), commandForm);
                    
                    if(commandResult.hasErrors()) {
                        ExecutionResult executionResult = commandResult.getExecutionResult();
                        
                        if(executionResult != null) {
                            EditMessageTypeDescriptionResult result = (EditMessageTypeDescriptionResult)executionResult.getResult();
        ...
        if(forwardKey.equals(ForwardConstants.FORM)) {
            ...
            request.setAttribute(AttributeConstants.MESSAGE_TYPE_NAME, messageTypeName);
            ...
            parameters.put(ParameterConstants.MESSAGE_TYPE_NAME, messageTypeName);
