...
@SproutAction(
    path = "/CancellationPolicy/CancellationKind/DescriptionEdit",
    mappingClass = SecureActionMapping.class,
    name = "CancellationKindDescriptionEdit",
    properties = {
        @SproutProperty(property = "secure", value = "true")
    },
    forwards = {
        @SproutForward(name = "Display", path = "/action/CancellationPolicy/CancellationKind/Description", redirect = true),
        @SproutForward(name = "Form", path = "/cancellationpolicy/cancellationkind/descriptionEdit.jsp")
    }
)
    ...
    throws Exception {
        ...
        try {
            if(forwardKey == null) {
                ...
                EditCancellationKindDescriptionForm commandForm = CancellationPolicyUtil.getHome().getEditCancellationKindDescriptionForm();
                CancellationKindDescriptionSpec spec = CancellationPolicyUtil.getHome().getCancellationKindDescriptionSpec();
                ...
                if(wasPost(request)) {
                    CancellationKindDescriptionEdit edit = CancellationPolicyUtil.getHome().getCancellationKindDescriptionEdit();
                    
                    commandForm.setEditMode(EditMode.UPDATE);
                    commandForm.setEdit(edit);
                    edit.setDescription(actionForm.getDescription());
                    
                    CommandResult commandResult = CancellationPolicyUtil.getHome().editCancellationKindDescription(getUserVisitPK(request), commandForm);
                    
                    if(commandResult.hasErrors()) {
                        ExecutionResult executionResult = commandResult.getExecutionResult();
                        
                        if(executionResult != null) {
                            EditCancellationKindDescriptionResult result = (EditCancellationKindDescriptionResult)executionResult.getResult();
                    ...
                    if(result != null) {
                        CancellationKindDescriptionEdit edit = result.getEdit();
