...
@SproutAction(
    path = "/Core/CommandMessageType/Review",
    mappingClass = SecureActionMapping.class,
    properties = {
        @SproutProperty(property = "secure", value = "true")
    },
    forwards = {
        @SproutForward(name = "Display", path = "/core/commandmessagetype/review.jsp")
    }
)
public class ReviewAction
    ...
    public ActionForward executeAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
        ...
        CommandMessageTypeTransfer commandMessageType = null;
        
        if(!commandResult.hasErrors()) {
            var executionResult = commandResult.getExecutionResult();
            var result = (GetCommandMessageTypeResult)executionResult.getResult();
            
            commandMessageType = result.getCommandMessageType();
        }
        
        if(commandMessageType == null) {
            forwardKey = ForwardConstants.ERROR_404;
        } else {
            request.setAttribute(AttributeConstants.COMMAND_MESSAGE_TYPE, commandMessageType);
