Feature: Anonymous user adds customer with login
  ...
  Scenario: Anonymous user adds a customer with login and an employee deletes the login
    Given the anonymous user Gerold adds a new customer with the first name "Gerold" and the last name "Jonas" and the email address "gj-test@echothree.com" and does allow solicitations to it and the username "gj" and the password "my-password" and the recovery question PET_NAME and the answer "Mufasa"
