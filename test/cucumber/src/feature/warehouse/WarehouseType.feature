Feature: Employee warehouse type
  ...
  Scenario: Existing employee adds a warehouse type, edits it, and then deletes it
    Given the employee Test begins using the application
    And the user begins entering a new warehouse type
    ...
    Then no error should occur
    And the user begins specifying a warehouse type to edit
    And the user sets the warehouse type's name to the last warehouse type added
    And the user begins editing the warehouse type
    Then no error should occur
    ...
    Then no error should occur
    And the user begins deleting a warehouse type
    And the user sets the warehouse type's name to the last warehouse type added
