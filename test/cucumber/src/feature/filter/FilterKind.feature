Feature: Employee entity attribute
  An employee wants to add, edit, and delete an filter kind
  ...
  Scenario: Existing employee adds an filter kind, edits it, and then deletes it
    Given the employee Test begins using the application
    ...
    Then no error should occur
    And the user begins specifying an filter kind to edit
    ...
    Then no error should occur
    And the user begins deleting an filter kind
    And the user sets the filter kind's name to the last filter kind added
