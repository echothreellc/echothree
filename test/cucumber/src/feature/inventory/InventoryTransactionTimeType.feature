Feature: Employee inventory transaction time type
  ...
  Scenario: Existing employee adds an inventory transaction time type, edits it, changes its status, and then deletes it
    ...
    And the user sets the inventory transaction time type's inventory transaction time type name to "CucumberInventoryTransactionTimeTypeName"
    ...
    And the user sets the inventory transaction time type's description to "Cucumber Inventory Transaction Time Type Name"
    ...
    And the user sets the inventory transaction time type's description to "Edited Cucumber Inventory Transaction Time Type Name"
  ...
  Scenario: Existing employee adds an inventory transaction time type, edits it by entity ref, and then deletes it by entity ref
    Given the employee Test begins using the application
    When the user begins entering a new inventory transaction time type
    And the user sets the inventory transaction time type's inventory transaction type name to "TEST_INVENTORY_TRANSACTION_TYPE"
    And the user sets the inventory transaction time type's inventory transaction time type name to "CucumberInventoryTransactionTimeTypeRef"
    And the user sets the inventory transaction time type to not be the default
    And the user sets the inventory transaction time type's sort order to "10"
    And the user sets the inventory transaction time type's description to "Cucumber Inventory Transaction Time Type Ref"
    And the user adds the new inventory transaction time type
    Then no error should occur
    When the user begins specifying an inventory transaction time type to edit
    ...
    Then no error should occur
    And the user sets the inventory transaction time type's sort order to "20"
    And the user sets the inventory transaction time type's description to "Edited Cucumber Inventory Transaction Time Type Ref"
    And the user finishes editing the inventory transaction time type
    Then no error should occur
    When the user begins deleting an inventory transaction time type
    And the user sets the inventory transaction time type's entity ref to the last inventory transaction time type added
