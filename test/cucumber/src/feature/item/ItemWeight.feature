Feature: Employee item weight
  An employee wants to add, edit, and delete an item weight
  ...
  Scenario: Existing employee adds two item unit of measure type, edits one of them, and then deletes both
    ...
    And the user sets the item weight's item to the last item added
    And the user sets the item weight's unit of measure type to "EACH"
    And the user sets the item weight's item weight type to "EXAMPLE_ITEM_WEIGHT_TYPE"
    And the user sets the item weight's weight unit of measure type to "OUNCE"
    And the user sets the item weight's weight to "42"
    And the user adds the new item weight
    Then no error should occur
    ...
    Then no error should occur
    And the user sets the item weight's weight unit of measure type to "POUND"
    And the user sets the item weight's weight to "1"
    ...
    And the user sets the item weight's unit of measure type to "EACH"
    And the user sets the item weight's item weight type to "EXAMPLE_ITEM_WEIGHT_TYPE"
    And the user deletes the item weight
    Then no error should occur
