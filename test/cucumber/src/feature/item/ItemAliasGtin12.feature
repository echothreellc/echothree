Feature: Employee GTIN-12 item alias
  An employee wants to add, edit, and delete an item alias that is a GTIN-12
  ...
  Scenario: Existing employee adds an item, then a valid GTIN-12 item alias, edits it, and deletes it
    Given the employee Test begins using the application
    ...
    And the user sets the item alias's item alias type to "GTIN_12"
    And the user sets the item alias's alias to "036000291452"
  ...
  Scenario: Existing employee adds an item, then tries to add a GTIN-12 item alias with an incorrect checksum
    Given the employee Test begins using the application
    When the user begins entering a new item
    And the user sets the item's type to REGULAR
    And the user sets the item's use type to REGULAR
    ...
    And the user sets the item alias's unit of measure type to "EACH"
    And the user sets the item alias's item alias type to "GTIN_12"
    And the user sets the item alias's alias to "036000291453"
    And the user adds the new item alias
    Then the execution error IncorrectGtin12Checksum should occur
    When the user sets the status of the last item added to ACTIVE_TO_DISCONTINUED
    Then no error should occur

  Scenario: Existing employee adds an item, then tries to add a GTIN-12 item alias with an incorrect character
    Given the employee Test begins using the application
    When the user begins entering a new item
    And the user sets the item's type to REGULAR
    And the user sets the item's use type to REGULAR
    ...
    And the user sets the item alias's unit of measure type to "EACH"
    And the user sets the item alias's item alias type to "GTIN_12"
    And the user sets the item alias's alias to "03600029A452"
    And the user adds the new item alias
    Then the execution error IncorrectGtin12Character should occur
    When the user sets the status of the last item added to ACTIVE_TO_DISCONTINUED
    Then no error should occur

  Scenario: Existing employee adds an item, then tries to add a GTIN-12 item alias with an incorrect length
    Given the employee Test begins using the application
    When the user begins entering a new item
    And the user sets the item's type to REGULAR
    And the user sets the item's use type to REGULAR
    ...
    And the user sets the item alias's unit of measure type to "EACH"
    And the user sets the item alias's item alias type to "GTIN_12"
    And the user sets the item alias's alias to "03600029145"
    And the user adds the new item alias
    Then the execution error IncorrectGtin12Length should occur
    When the user sets the status of the last item added to ACTIVE_TO_DISCONTINUED
    Then no error should occur
