Feature: Employee UPC-A item alias
  An employee wants to add, edit, and delete an item alias that is a UPC-A
  ...
  Scenario: Existing employee adds an item, then a valid UPC-A item alias, edits it, and deletes it
    Given the employee Test begins using the application
    ...
    And the user sets the item alias's item alias type to "UPC_A"
  ...
  Scenario: Existing employee adds an item, then tries to add a UPC-A item alias with an incorrect checksum
    Given the employee Test begins using the application
    When the user begins entering a new item
    And the user sets the item's type to REGULAR
    And the user sets the item's use type to REGULAR
  ...
  Scenario: Existing employee adds an item, then tries to add a UPC-A item alias with an incorrect character
    Given the employee Test begins using the application
    When the user begins entering a new item
    And the user sets the item's type to REGULAR
    And the user sets the item's use type to REGULAR
    ...
    And the user adds the new item alias
    Then the execution error IncorrectUpcACharacter should occur
    When the user sets the status of the last item added to ACTIVE_TO_DISCONTINUED
    Then no error should occur

  Scenario: Existing employee adds an item, then tries to add a UPC-A item alias with an incorrect length
    Given the employee Test begins using the application
    When the user begins entering a new item
    And the user sets the item's type to REGULAR
    And the user sets the item's use type to REGULAR
    ...
    And the user adds the new item alias
    Then the execution error IncorrectUpcALength should occur
    When the user sets the status of the last item added to ACTIVE_TO_DISCONTINUED
    Then no error should occur
